...
当前项目主要用于公开演示，目的是模拟市场监管部门工单系统如何接入智能体能力。Demo 阶段不会真实调用市场监管部门工单系统接口，流转、退单、写回补充任务均为模拟动作。
...
后续如果面向市场监管部门自有环境或公司专网部署，需要重点关注：

- 不要把真实 API Key 提交到 Git。
- `.env` 要由部署环境单独维护。
- 大模型、embedding、reranker、PostgreSQL 应确认网络可达。
- 如果数据不得出自有环境，不建议把自有环境模型 API 配到外部 IDE 或第三方客户端。
- LLM 输出必须保留 schema 校验、置信度阈值和审计信息。
- 退单动作必须人工确认，不应完全自动化。
- 自动流转前要记录模型输出、置信度、prompt 版本、处理时间和最终动作。
- 真实接入市场监管部门系统时，`actions.py` 中的模拟动作需要替换为实际工单系统接口调用。
- 法律知识库应建立导入、版本、审核和更新机制。
